 ...
 * @return {void}
 */
async function initDataLoad(jump=true) {
    load.open();
    var url = '../sample/login.json?t=' + new Date().getTime();
    try {
...
async function submitLoginForm() {
    ...
    try {
        load.open();
        var res = await fetch("../sample/login.json?t=" + new Date().getTime(), {
        });
        var json = await res.json();
    } catch (error) {
        Swal.fire({
...
Swal.fire({
    ...
    text: '留言板内容为虚构，仅演示用，欢迎Star，点击下方msgboard按钮前往留言板演示页',
    timer: 10000,
